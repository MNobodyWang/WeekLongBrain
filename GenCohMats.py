...
def GenCohMats(MEG_Dir,subjID,recordInd,aCorr,aveCortex,startInd):
    dirList=glob.glob(MEG_Dir+"/ElectrodeTrials/"+subjID+"/*")
    groupList = [os.path.basename(x) for x in dirList]
    readDir=MEG_Dir+"/ElectrodeTrials/"+subjID+"/"+groupList[recordInd]+"/"
