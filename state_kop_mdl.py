...
class state_kop_mdl:
	...
	def make_dec_model(self):
		model = tf.keras.Sequential()
		model.add(layers.Dense(self.kop_states, input_shape=(self.kop_states,)))
		model.add(layers.ReLU())
		model.add(layers.Dropout(0.1))
	...
	def train_step(self,x_tr,y_tr):
		with tf.GradientTape() as enc_tape, tf.GradientTape() as dec_tape, tf.GradientTape() as kop_tape:
			...
			step_loss = self.loss_fn(y_tr_enc,y_tr_enc_pred)+self.loss_fn(x_tr,x_tr_pred)
